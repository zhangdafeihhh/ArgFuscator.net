class SedStatement {
    private static FORMAT = new RegExp(/^s\/((?:[^\/\\]|\\(?:\\|\/))+?)\/((?:[^\/\\]|\\(?:\\|\/))*?)\/([ig])?$/)
    public Find: string;
    ...
    constructor(Statement: string) {
        ...
        this.Find = results[1].replace(/\\(\\|\/)/g, '$1'); // The string to look for
        this.Replace = results[2].replace(/\\(\\|\/)/g, '$1').split('|'); // One can specify multiple replacement options, separated by a pipe character
